...
#define EXAMPLE_WIFI_SSID            "Home_WiFi"
#define EXAMPLE_WIFI_PASS            "Stefany21062009wf"
...
void socket_server(void *ignore) {
	...
	while (1) {
		ESP_LOGI(tag, "In srv loop");
		// Listen for a new client connection.
		socklen_t clientAddressLength = sizeof(clientAddress);
		int clientSock = accept(sock, (struct sockaddr *)&clientAddress, &clientAddressLength);
		ESP_LOGI(tag, "In srv loop2");
		if (clientSock < 0) {
			ESP_LOGI(tag, "In srv loop er1");
			ESP_LOGE(tag, "accept: %d %s", clientSock, strerror(errno));
			goto END;
		}
		ESP_LOGI(tag, "In srv loop3");
...
void app_main(void)
{
	...
	xTaskCreate(socket_server, "s_sr", 2048, NULL, 5, NULL);
}
